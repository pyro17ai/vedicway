...
| BE-05 | Покупка, webhook, entitlement, recovery email и magic link | частично | Реализованы server catalog 99000 RUB, async YooKassa API v3 adapter, `capture=true`, email-чек, сохранённый `Idempotence-Key`, fail-closed production settings и локальный simulator за `VEDICWAY_TEST_PAYMENTS=1`. Webhook ограничен официальными сетями YooKassa и подтверждает payment/refund через API; provider ID, amount, currency и metadata сверяются до транзакционного entitlement. После первого готового PDF ставится одно дедуплицированное транзакционное письмо на покупку; `POST /api/v1/access/recovery` всегда отвечает одинаково, ищет покупку по HMAC email и выдаёт одноразовые scoped-ссылки `read_chart` и `download_pdf`, привязанные к точному `render_request_id`. Очередь обслуживает отдельная роль `email` с data/signing/SMTP secrets и изолированной egress-сетью; API и расчётный worker SMTP-пароль не получают. HTTPS/TLS-конфигурация проверяется preflight, bearer-route исключён из access log. Код и контрактные тесты готовы, но живая доставка остаётся закрытой до выдачи production credentials, проверки домена и SPF/DKIM/DMARC. |
...
| BE-07 | Приватность, ограничения, наблюдаемость и lifecycle-контуры | частично | Birth/email/note и причины возвратов шифруются, email lookup хранится как HMAC, доступ проверяется на каждом chart route, subprocess запускается без shell, PII не попадает в log labels. Реализованы owned `DELETE /api/v1/charts/{chart_id}`, одинаковый intake `POST /api/v1/privacy/requests`, атомарная очистка runtime-данных с tombstone, псевдонимизация email в финансовой записи и bounded unlink только внутри reports-каталога. Идемпотентный lifecycle доступен через сетево изолированные Compose one-shot сервисы `retention-dry-run`/`retention-apply`; runbook фиксирует ежедневный systemd timer. Production readiness требует пять явно утверждённых сроков хранения. Юридические сроки, внешний backup/tombstone restore-контур и подтверждение реального SMTP-домена остаются release gates. |
| BE-08 | Unit, golden, API, runner, payment, load и PDF-проверки | частично | Backend regression/integration-набор покрывает timezone, golden D1/D9/D10, активную махадашу и антардашу, evidence uniqueness, изоляцию и repair runner, payment idempotency, YooKassa adapter через MockTransport, webhook source verification, reconcile, partial/full refund, entitlement, PDF и magic link. Playwright проверяет локальный redirect simulator, восстановление оплаты после reload и PDF. Нагрузочные 10/50/200, 100 agent eval и живой тестовый магазин YooKassa требуют staging и авторизации владельца, поэтому остаются перед production rollout. |
