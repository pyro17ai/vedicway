...
| BE-04 | Interpretation provider, JSON Schema и semantic validation | готово | `CodexExecProvider` запускает зафиксированный Codex CLI 0.144.6 как isolated one-shot процесс с `--ephemeral`, dedicated `CODEX_HOME`, пустым read-only workdir, отключёнными web search, MCP, apps и shell. Prompt `interpretation-editor-ru.v2` получает только обезличенные evidence packets без внутреннего `snapshot_id`; фактический идентификатор восстанавливает сервер после валидации ответа. Free и paid используют разные модели и reasoning из env. Строгая JSON Schema дополняется semantic validator: проверяются порядок восьми domains, обязательные D1/D9/D10/D2/D4/D24, coverage, evidence IDs, персональность заголовков, повторы, платный объём и запрещённые утверждения. Один repair допустим; при ошибке Codex карта D1 остаётся доступной, а шаблонная интерпретация не подставляется. Локальный HTTP smoke 18.07.2026 создал валидный `interpretation.free.v1` с первого model response за 32,5 с; производственный p95 ≤15 с остаётся отдельным performance gate. |
| BE-05 | Покупка, webhook, entitlement, recovery email и magic link | частично | Реализованы server catalog 99000 RUB, async YooKassa API v3 adapter, `capture=true`, email-чек, сохранённый `Idempotence-Key`, fail-closed production settings и локальный simulator за `VEDICWAY_TEST_PAYMENTS=1`. Webhook ограничен официальными сетями YooKassa и подтверждает payment/refund через API; provider ID, amount, currency и metadata сверяются до транзакционного entitlement. После первого готового PDF ставится одно дедуплицированное транзакционное письмо на покупку; recovery ищет покупку по HMAC email и выдаёт одночасовые scoped-ссылки `read_chart` и `download_pdf`, привязанные к точному `render_request_id`. Первый GET создаёт только десятиминутный preauth, явный same-origin POST атомарно расходует ссылку и выдаёт сессию; scanner replay получает отказ. Uvicorn и Nginx не пишут URI magic-link, а внешний TLS ingress обязан редактировать его до экспорта журналов. Очередь обслуживает отдельная роль `email` с изолированными SMTP secrets. Код и браузерные тесты готовы, но живая доставка и оплата остаются закрытыми до production credentials, проверки домена, SPF/DKIM/DMARC и тестового магазина YooKassa. |
| BE-06 | Playwright PDF worker, storage и signed download | готово | PDF строится из сохранённых snapshot и bundle: trusted escaped HTML, выбранные пользователем `varga/mode`, Chromium A4, random object key, checksum, pages и short-lived signed download. Отдельный `render_request_id` фиксирует настройки в момент нажатия и исключает подмену параметров последующим переключением интерфейса. |
| BE-07 | Приватность, ограничения, наблюдаемость и lifecycle-контуры | частично | Birth/email/note и причины возвратов шифруются, email lookup хранится как HMAC, доступ проверяется на каждом chart route, subprocess запускается без shell, PII не попадает в log labels. Политика и отдельное согласие раскрывают фактического обработчика объяснений, цель, категории данных и трансграничность; при Codex provider readiness не принимает пустые значения или placeholders. Реализованы owned deletion, нейтральный privacy intake, атомарная очистка с tombstone, псевдонимизация финансовой записи и bounded unlink. Paired backup шифрует согласованные PostgreSQL/runtime snapshots AES-256-GCM, а restore сначала проверяет оба компонента и переключает их как одну staged-версию. Lifecycle доступен через изолированные one-shot сервисы, production readiness требует пять утверждённых сроков хранения. Юридическое утверждение сроков, договоров с обработчиками, уведомлений и реальная restore-тренировка остаются организационными release gates. |
| BE-08 | Unit, golden, API, runner, payment, load и PDF-проверки | частично | Финальный набор проходит `108/108` backend-тестов, Ruff, компиляцию Python и `pip-audit` без известных уязвимостей на pytest 9.1.1. Regression/integration покрывает timezone, golden D1/D9/D10, активную махадашу и антардашу, evidence uniqueness, isolation/repair runner, payment idempotency, YooKassa MockTransport, webhook verification, reconcile, refunds, entitlement, PDF, media visibility и двухшаговый magic-link. Playwright `13/13` проверяет redirect simulator, оплату после reload, точный PDF и recovery в настоящем Chromium. Нагрузочные 10/50/200, 100 agent eval и живой тестовый магазин YooKassa требуют staging и авторизации владельца, поэтому остаются перед production rollout. |
