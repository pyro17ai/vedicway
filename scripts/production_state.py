...
SERVICES = ("frontend", "backend", "worker", "email")
...
def _resume(base: list[str], env: dict[str, str]) -> None:
    _run(base, "up", "-d", *SERVICES, env=env)
...
def backup(base: list[str], env: dict[str, str]) -> str:
    ...
    _run(base, "stop", *SERVICES, env=operation_env)
    try:
...
def restore(base: list[str], env: dict[str, str], bundle: str) -> None:
    ...
    try:
        ...
        _run(base, "--profile", "ops", "run", "--rm", "restore", env=finalize_env)
        _run(base, "--profile", "ops", "run", "--rm", "runtime-restore", "finalize", env=finalize_env)
    except Exception:
        ...
        if postgres_committed and runtime_committed:
            _resume(base, operation_env)
