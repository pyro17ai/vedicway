...
async function expectStableFaqHeight(page: Page) {
  ...
  const initialRows = await triggers.evaluateAll((elements) => elements.map((element) => {
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    const currentRows = await triggers.evaluateAll((elements) => elements.map((element) => {
      const rect = element.getBoundingClientRect();
      return { top: rect.top + window.scrollY, height: rect.height };
    }));
    const currentPageHeight = await page.evaluate(() => document.documentElement.scrollHeight);

    expect(Math.abs(currentHeight - initialHeight)).toBeLessThanOrEqual(0.5);
    expect(currentRows).toEqual(initialRows);
    expect(currentPageHeight).toBe(initialPageHeight);
