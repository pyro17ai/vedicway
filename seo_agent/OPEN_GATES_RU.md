...
Локальная проверка Docker-образа и `docker compose config` на этой машине не выполнена: Docker CLI здесь отсутствует. GitHub Actions содержит эту проверку для Linux, а на сервере ее надо пройти до включения профиля `seo`.
