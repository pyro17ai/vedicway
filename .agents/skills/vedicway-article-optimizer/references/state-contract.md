...
Terminal `publication-attempt` содержит `draft_id`, `target=site`, `claim_token`, стабильный `idempotency_key`, уникальный `attempt_token`, `status`, `content_hash`, `request_hash` и очищенный `response`. `publication` повторяет `claim_token`, `attempt_token`, оба hash и объект `evidence.checks` с истинными `canonical`, `article_schema`, `title`, `request_hash`, `sitemap`, `cover`.
